...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const { data: existing } = await db.from("Quote").select("meta, lineItems").eq("id", params.id).single();
  ...
  if (body.lineItems !== undefined) {
    ...
    update.total = total;
    update.proFees = total;
    update.services = items.map((i) => i.description).join(", ");
  }

  const metaPatch: Record<string, unknown> = {};
  for (const k of ["docType", "subject", "clientRefId", "referenceNo"] as const) {
    if (body[k] !== undefined) metaPatch[k] = body[k];
  }
  if (Object.keys(metaPatch).length) update.meta = { ...prevMeta, ...metaPatch };
  if (body.status !== undefined) update.status = body.status;

  const { data, error } = await db.from("Quote").update(update).eq("id", params.id).select().single();
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  ...
  const db = createAdminClient();
  const { error } = await db.from("Quote").delete().eq("id", params.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
